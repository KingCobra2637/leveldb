...
// Set to true on the thread currently running ForceFullCompaction so its own
// internal calls to Write()/Get() skip the gate (otherwise it would deadlock
// waiting on its own gate).
namespace {
...
Status DBImpl::ForceFullCompaction() {
  // Wait for any other ForceFullCompaction to finish, then raise the gate.
  {
    MutexLock l(&mutex_);
  ...
  t_is_force_compaction_thread_ = true;

  // Snapshot per-level stats so we can report just this call's work.
  ...
  // Flush memtable, then compact every level into the next.
  Status s = TEST_CompactMemTable();
  for (int level = 0; s.ok() && level + 1 < config::kNumLevels; ++level) {
    TEST_CompactRange(level, nullptr, nullptr);
    MutexLock l(&mutex_);
    if (!bg_error_.ok()) s = bg_error_;
  }

  // Diff stats and accumulate into a single total.
  CompactionStats total;
  {
    MutexLock l(&mutex_);
    for (int i = 0; i < config::kNumLevels; i++) {
      CompactionStats d;
      d.micros = stats_[i].micros - before[i].micros;
      d.bytes_read = stats_[i].bytes_read - before[i].bytes_read;
      d.bytes_written = stats_[i].bytes_written - before[i].bytes_written;
      d.num_compactions = stats_[i].num_compactions - before[i].num_compactions;
      d.num_input_files = stats_[i].num_input_files - before[i].num_input_files;
      d.num_output_files =
  ...
  printf("ForceFullCompaction summary (%.3f s):\n"
         "  compactions executed: %lld\n"
         "  input files:          %lld\n"
         "  output files:         %lld\n"
         "  bytes read:           %lld\n"
         "  bytes written:        %lld\n",
         (env_->NowMicros() - start_micros) / 1e6,
         (long long)total.num_compactions,
         (long long)total.num_input_files,
         (long long)total.num_output_files,
         (long long)total.bytes_read,
         (long long)total.bytes_written);
  fflush(stdout);

  // Lower the gate and wake any threads parked on it.
  t_is_force_compaction_thread_ = false;
  {
    MutexLock l(&mutex_);
    force_compaction_in_progress_ = false;
...
Iterator* DBImpl::NewInternalIterator(const ReadOptions& options,
                                      ...
                                      uint32_t* seed) {
  mutex_.Lock();
  // Wait for any in-progress ForceFullCompaction.
...
Status DBImpl::Write(const WriteOptions& options, WriteBatch* updates) {
  ...
  MutexLock l(&mutex_);
  // Wait for any in-progress ForceFullCompaction (must happen before joining
  // writers_ so a blocked writer never occupies a queue slot).
  while (force_compaction_in_progress_ && !t_is_force_compaction_thread_) {
    force_compaction_done_signal_.Wait();
  }
